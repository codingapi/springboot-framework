...
自 `8.2.0` 起，项目采用 CI-Friendly 版本管理方式（与 [flow-engine](https://github.com/codingapi/flow-engine) 相同）：pom 中的版本声明为 `${revision}`，开发期锁定为 SNAPSHOT 版本（当前为 `8.2.0-SNAPSHOT`），正式发布时通过 `-Drevision=x.y.z` 指定正式版本号，无需修改 pom。
